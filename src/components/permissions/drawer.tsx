import { Checkbox, Descriptions, Form, Input, Tabs, Typography } from 'antd';
...
const RoleDrawer: React.FC<IProps> = ({ data: editData, payload, setPayload }) => {
	...
	return (
		<Fragment>
			<Form.Item rules={[{ required: true, message: 'Name is required' }]} name="name" label="Name">
				<Input required value={payload?.displayName} onChange={(e) => updateName(e.target.value)} />
			</Form.Item>
			...
			<Tabs
				...
				items={Object.entries(payload?.permissions ?? {}).map(([resourceName, resPermissions]) => ({
					key: resourceName,
					value: resourceName,
